...
impl Population {
    ...
    pub fn new(population_count: usize,
               graph: Box<Graph>,
               mutation_rate: f64,
               tournament_size: usize,
               mut rng: StdRng)
               -> Population {
        let population =
            (0..population_count).map(|_| Tour::random_tour(&mut rng, &*graph)).collect::<Vec<_>>();
        Population {
            rng: rng,
            graph: graph,
    ...
    pub fn evolve(&mut self) -> Population {
        ...
        let mutated = new_population.iter()
            .map(|tour| tour.mutate(&mut self.rng, &*self.graph, self.mutation_rate))
            .collect();
        // println!("{}\n", mutated)
        Population {
            rng: self.rng,
